    ...
    public function getApprovedLeavesByCurrentYear(Request $request)
{
    ...
    public function updateLeave(Request $request)
    {
        ...
        $leave = Leave::with('User')->find($request->leave_id);

        // Check if leave exists and belongs to the user
        if (!$leave || $leave->employee_id != $user->id) {
            return response()->json([
                'status' => 'error',
        ...
        if ($leave->status != 'Pending') {
            return response()->json([
                'status' => 'error',
                'message' => 'Leave can only be updated when status is Pending'
            ], 400);
        }
        ...
        $this->addLogActivity([
            ...
            'note' => json_encode([
                ...
                'message' => $leave->user->name . ' leave updated'
            ]),
            'module_id' => $leave->id,
            'module_type' => 'leave',
        ...
        // Return success response
        return response()->json(['success' => __('Leave successfully updated.')], 200);
    }
